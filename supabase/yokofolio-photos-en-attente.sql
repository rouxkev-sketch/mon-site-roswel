-- =====================================================================
--  MIGRATION Nº 70 — LES PHOTOS NEUVES ATTENDENT, LA FICHE RESTE EN LIGNE
--  (passe nº 285 · règles 3 et 6)
-- =====================================================================
--  À LANCER DANS L'ÉDITEUR SQL DE SUPABASE, EN UNE FOIS.
--  Elle ajoute UNE colonne et refait la fonction `rechercher_tatoueurs`.
--  Elle ne détruit AUCUNE donnée. Se relance sans risque.
--
--  ⚠️ PASSER LA Nº 69 AVANT CELLE-CI : ce fichier repart de son corps
--  (la coupe par carrousel) et n'y ajoute QUE l'exclusion des photos en
--  attente. La vue `popularite_tatoueurs` vient de la nº 62 ; ce
--  fichier ne la recrée pas.
--
--  CE QU'ELLE INSTALLE, ET POURQUOI
--  -------------------------------------------------------------
--  Jusqu'ici, TOUTE modification d'une fiche en ligne partait dans un
--  `brouillon` et attendait une relecture — la bio comme les liens,
--  alors que les photos, elles, s'écrivaient DIRECTEMENT et
--  s'affichaient aussitôt. C'était l'inverse de ce qu'il faut : ce
--  qu'on relit, ce sont LES IMAGES.
--
--  LES SIX RÈGLES DE LA PASSE Nº 285 renversent cela :
--   · la première création passe la validation (règle 1) ;
--   · ensuite, TOUTE modification part en ligne (règle 2) ;
--   · SAUF les photos qui ARRIVENT — nouvelles photos d'un carrousel
--     existant, ou nouveau style et ses photos (règle 3) ;
--   · réordonner ne déclenche rien (règle 5) ;
--   · et pendant l'attente LA FICHE RESTE EN LIGNE, telle qu'elle
--     était : seules les photos en attente sont invisibles (règle 6).
--
--  UNE COLONNE SUFFIT À TENIR LA RÈGLE 6 :
--  `photos_tatoueur.en_attente`. Une photo qui arrive sur une fiche
--  déjà validée est écrite `true` ; son auteur la voit dans son
--  formulaire, LE PUBLIC NE LA VOIT PAS. « Valider » dans l'écran
--  d'administration la repasse à `false`, et elle paraît.
--  ⚠️ ON NE TOUCHE NI À `publie`, NI À `statut` : la fiche n'est
--  jamais retirée du site, jamais remise dans la file entière.
--
--  DEUX LECTURES À FILTRER, ET ELLES LE SONT TOUTES LES DEUX :
--   · LE CHEMIN RAPIDE — `rechercher_tatoueurs`, refaite ici : la
--     clause qui ramasse les photos écarte celles qui attendent ;
--   · LE CHEMIN DE SECOURS ET LA PAGE D'UNE FICHE — filtrés côté site
--     (`garnirFiches`, src/lib/tatoueurs.ts), qui lisent la colonne.
--
--  ⚠️ LE SITE N'EXIGE PAS CETTE MIGRATION. Sans elle, la colonne
--  n'existe pas : l'écriture des photos neuves est rejouée sans elle,
--  la lecture ne trouve rien à filtrer, et tout se comporte comme
--  avant la nº 285 — les photos neuves sont visibles tout de suite.
--  Aucune version du site ne dépend de l'autre.
-- =====================================================================

-- ------------------------------------------------------------
--  1) LA COLONNE — une photo attend-elle sa relecture ?
-- ------------------------------------------------------------
alter table public.photos_tatoueur
  add column if not exists en_attente boolean not null default false;

comment on column public.photos_tatoueur.en_attente is
  'Passe nº 285, règle 3 : cette photo vient d''arriver sur une fiche DÉJÀ validée et attend une relecture. Elle est visible de son auteur (son formulaire), JAMAIS du public (mosaïque, fiche, partage). « Valider » dans l''écran d''administration la repasse à false. Une photo déplacée, retaguée ou retirée ne touche jamais cette colonne (règle 5).';

--  L'index sert L'ÉCRAN D'ADMINISTRATION, qui demande « quelles fiches
--  ont des photos en attente ? ». Partiel : il ne pèse que ce que
--  pèsent les photos qui attendent — presque rien, presque toujours.
create index if not exists photos_tatoueur_en_attente_idx
  on public.photos_tatoueur (tatoueur_id)
  where en_attente;

-- ------------------------------------------------------------
--  2) LA RECHERCHE — elle ne ramasse plus les photos en attente
-- ------------------------------------------------------------
          ...
          from (
            select p3.*,
             ...
             where p3.tatoueur_id = s.id
               --  ⚠️ UNE PHOTO QUI ATTEND SA RELECTURE N'EXISTE PAS
               --  POUR LE PUBLIC (migration nº 70, règle 6 de la
               --  nº 285). Elle est en base, son auteur la voit dans
               --  son formulaire — la mosaïque, la fiche et le partage
               --  ne la voient pas. `coalesce` : les lignes d'avant
               --  cette migration valent « pas en attente ».
               and not coalesce(p3.en_attente, false)
          ) p2
...
comment on function public.rechercher_tatoueurs is
  'LA recherche de yokofolio : filtre, distance, tri, limite — tout en base. Depuis la nº 42 elle interroge TOUS les lieux d''une fiche ; depuis la nº 49 elle connaît la NATURE des photos (tatouage / flash) ; depuis la nº 58 LE STYLE SE LIT DANS LES PHOTOS CATALOGUÉES ; depuis la nº 61 L''ORDRE DES RÉSULTATS NE DÉPEND PLUS DE LA TAILLE DE LA PAGE ; depuis la nº 62 elle intègre un SCORE DE POPULARITÉ ; depuis la nº 63 ce score est dans LES DEUX `order by` ; depuis la nº 66 ELLE NE MASQUE PLUS AUCUN COMPTE ; depuis la nº 69 ELLE NE COUPE PLUS PAR FICHE MAIS PAR CARROUSEL — au plus dix photos par groupe style · nature · rendu ; depuis la nº 70 ELLE ÉCARTE LES PHOTOS QUI ATTENDENT LEUR RELECTURE (`photos_tatoueur.en_attente`) : une fiche en ligne le reste pendant qu''une photo neuve attend, et seule cette photo est invisible.';
...
from (values
  (1, 'La colonne en_attente existe (doit être « oui »)', (
    select case when exists (
      select 1 from information_schema.columns
       where table_schema = 'public'
         and table_name = 'photos_tatoueur'
         and column_name = 'en_attente')
      then 'oui' else 'NON' end)),
  (2, 'La recherche écarte les photos en attente (doit être « oui »)', (
    select case when exists (
      select 1 from pg_proc
       where proname = 'rechercher_tatoueurs'
         and prosrc like '%en_attente%')
      then 'oui' else 'NON' end)),
  (3, 'Elle coupe toujours PAR CARROUSEL, acquis de la nº 69 (doit être « oui »)', (
    select case when exists (
      select 1 from pg_proc
       where proname = 'rechercher_tatoueurs'
  ...
  (5, 'Photos qui attendent une relecture, aujourd''hui (doit être 0 juste après la migration)', (
    select count(*)::text from public.photos_tatoueur where en_attente)),
