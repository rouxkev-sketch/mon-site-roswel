...
/**
 * ██ §1 et §2 (nº 390) — L'ÉTAT D'UNE SURFACE RECOPIE CELUI D'EN
 * DESSOUS ██
 * ==================================================================
 * LE RELEVÉ DU PROPRIÉTAIRE, SYSTÉMATIQUE : un seul clic sur une carte
 * produisait DEUX lignes POSÉE à la même adresse, et chaque fermeture
 * déchargeait le document entier (« DÉPART DU SITE » puis « DOCUMENT
 * OUVERT »).
 *
 * UNE SEULE CAUSE, ET ELLE ÉTAIT DÉJÀ ÉCRITE — DANS LE FILET. Le
 * commentaire de `RetourGaranti` pose le contrat noir sur blanc :
 * « Une surface refermable pose SON étape par-dessus la nôtre, et son
 * état RECOPIE le nôtre : tant que la marque `retourReconstruit` est
 * encore là, c'est qu'on est AU-DESSUS du filet — il ne bouge pas. »
 * OR AUCUNE DES QUATRE SURFACES NE RECOPIAIT QUOI QUE CE SOIT : la
 * fenêtre de fiche de la mosaïque, celle de « Ma sélection », la pile
 * de fiches et la fenêtre de carrousel écrivaient toutes un état NEUF
 * et plat (`{ fenetreFiche: true }`), qui EFFACE la marque du filet.
 *
 * CE QUE ÇA PRODUISAIT, DANS L'ORDRE :
 *  1. le clic pousse l'étape de la fenêtre — état plat, marque du
 *     filet perdue ;
 *  2. le filet, armé plus tard dans le même document, ne se voit plus
 *     en dessous : il se repose PAR-DESSUS l'étape de la fenêtre, en
 *     recopiant cette fois l'état qu'il trouve — d'où une SECONDE
 *     ligne POSÉE portant le badge « fenêtre de fiche », à la même
 *     adresse. C'est le point 1 ;
 *  3. au retour, on retombe sur l'étape de la fenêtre. Elle ne porte
 *     PAS `retourReconstruit` : le filet croit que le retour l'a
 *     dépassé par le bas et exécute `location.replace("/")` — le
 *     document est déchargé et rechargé. C'est le point 2.
 * Les deux symptômes sont donc le même défaut, vu de deux endroits.
 *
 * CETTE FONCTION EST LE CONTRAT, RENDU EXÉCUTABLE : elle part de
 * l'état COURANT et n'ajoute que les marques de la surface. Toute
 * marque posée en dessous — celle du filet aujourd'hui, une autre
 * demain — remonte donc d'elle-même, sans que la surface ait à la
 * connaître.
 *
 * ⚠️ ELLE N'ÉCRIT AUCUNE ADRESSE ET NE POSE AUCUNE ENTRÉE : elle rend
 * un objet, rien de plus. L'appelant garde son `pushState`, son
 * adresse et son compte d'entrées — la règle 332-§1 n'est pas touchée.
 */
export function etatDeSurface(
  marques: Record<string, unknown>
): Record<string, unknown> {
  const courant =
    typeof window === "undefined"
      ? null
      : (window.history.state as Record<string, unknown> | null);
  return { ...(courant ?? {}), ...marques };
}
